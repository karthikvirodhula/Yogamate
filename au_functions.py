...
def add_gif(pose):
    ...
    file_ = open(os.path.join(".", "finalgifs", f'{poses[pose]}'), "rb")  # Path to your GIF file
    contents = file_.read()
    data_url = base64.b64encode(contents).decode("utf-8")
    file_.close()
...
def instruction(pos,lang):
    path=os.path.join('.','languages','instructions',f'{lang}',f'{pos}.mp3')
